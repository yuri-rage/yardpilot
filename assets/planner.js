  ...
  if (typeof ClipperLib !== "undefined") {
    try {
      ...
      if (solution.length > 0) {
        ...
        return solution[bestIdx].map((pt) => [pt.X / scale, pt.Y / scale]);
      }
      return [];
    } catch (e) {
      console.warn(
...
function expandPoly(poly, dist) {
  ...
  if (typeof ClipperLib !== "undefined") {
    try {
      ...
      console.warn("Clipper offset failed, falling back to miter-offset:", e);
    }
  }
...
function routeTransition(
  ...
  if (useToleranceForDirect) {
    ...
    if (
      segmentFree(from, to, perimR, exR, exRC, perimBBox, exBBoxes, exCBBoxes)
    ) {
      return [from, to];
    }
  }
  ...
  for (const qi of [0, 1]) {
    ...
    const targetInTolerance =
      perimR_tolerance.length >= 3 && pointInPoly(target, perimR_tolerance);
        ...
        .map((s, idx) => {
          ...
          if (s.type === "circle") {
            ...
            return {
              ...
              poly: rotPts(
                circlePoly(cx, cy, s.radius + totalBuffer, circleSegments),
                -angle,
              ),
            };
          }
          ...
          const expanded =
            totalBuffer > 0 ? expandPoly(polyLocal, totalBuffer) : polyLocal;
          return {
            type: "polygon",
