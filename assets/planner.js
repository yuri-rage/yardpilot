...
function _eeTheta(phi) { return Math.asin(Math.sqrt(3) / 2 * Math.sin(phi)); }
...
function toLocal(lat, lon, oLat, oLon) {
    ...
    const x = R_EARTH * 2 * Math.sqrt(3) * lam * Math.cos(th) / (3 * _eeDY(th));
...
function fromLocal(x, y, oLat, oLon) {
    ...
    const lam = x * 3 * _eeDY(th) / (R_EARTH * 2 * Math.sqrt(3) * Math.cos(th));
...
function convexHull(pts) {
    ...
    const lo = [], hi = [];
    for (const p of s) {
        while (lo.length >= 2 && cross2d(lo.at(-2), lo.at(-1), p) <= 0) lo.pop();
        lo.push(p);
    }
    for (let i = s.length - 1; i >= 0; i--) {
        const p = s[i];
        while (hi.length >= 2 && cross2d(hi.at(-2), hi.at(-1), p) <= 0) hi.pop();
        hi.push(p);
    }
    hi.pop(); lo.pop();
    return lo.concat(hi);
...
function mbbSweepAngle(hull) {
    ...
    for (let i = 0; i < hull.length; i++) {
        ...
        if (area < minArea) { minArea = area; best = a; }
    }
    return best;
}
...
function crossesBoundary(a, b, poly, polyBBox = null) {
    const sMinX = Math.min(a[0], b[0]), sMaxX = Math.max(a[0], b[0]);
    const sMinY = Math.min(a[1], b[1]), sMaxY = Math.max(a[1], b[1]);

    if (polyBBox) {
        if (sMaxX < polyBBox.min[0] || sMinX > polyBBox.max[0] ||
            sMaxY < polyBBox.min[1] || sMinY > polyBBox.max[1]) {
            return false;
        }
    }
    ...
    for (let i = 0; i < poly.length; i++) {
        ...
        const eMinY = Math.min(p1[1], p2[1]), eMaxY = Math.max(p1[1], p2[1]);
...
function segmentFree(a, b, perim, excl, exclC, perimBBox = null, exclBBoxes = [], exclCBBoxes = []) {
...
function insetPoly(poly, dist) {
    ...
    for (let i = 0; i < N; i++) {
        ...
        // Edge vectors
        const dx1 = B[0] - A[0], dy1 = B[1] - A[1], len1 = Math.hypot(dx1, dy1);
        const dx2 = C[0] - B[0], dy2 = C[1] - B[1], len2 = Math.hypot(dx2, dy2);
...
function subtractPolygon(P_orig, E_orig) {
    ...
    let listP = [];
    for (let i = 0; i < N; i++) {
        listP.push({ pt: P[i], isIntersection: false, idx: i, intersections: [] });
    }
    
    let listE = [];
    for (let i = 0; i < M; i++) {
        listE.push({ pt: E[i], isIntersection: false, idx: i, intersections: [] });
    }
    ...
    for (let i = 0; i < N; i++) {
        ...
        for (let j = 0; j < M; j++) {
            ...
            if (inter) {
                const nodeP = { pt: inter.pt, isIntersection: true, t: inter.t, partner: null };
                const nodeE = { pt: inter.pt, isIntersection: true, t: inter.u, partner: null };
                nodeP.partner = nodeE;
                nodeE.partner = nodeP;
    ...
    if (!anyInter) {
        const E_in_P = pointInPoly(E[0], P);
...
function expandPoly(poly, dist) {
    ...
    return poly.map(([x, y]) => {
        ...
        return len < 1e-9 ? [x, y] : [x + dx / len * dist, y + dy / len * dist];
    });
}
...
function subtractIntervals(lo, hi, exXs) {
    ...
    for (let i = 0; i + 1 < exXs.length; i += 2) {
        ...
        for (const [sl, sr] of segs) {
            if (eR <= sl || eL >= sr) { next.push([sl, sr]); continue; }
...
function buildGrid(perimR, exR, cellSize) {
    const xs = perimR.map(p => p[0]), ys = perimR.map(p => p[1]);
...
function generateStrips({ grid, xMin, yMin, rows, cols }, cellSize, perimR, exR) {
    const strips = [];
    ...
    for (let r = 0; r < rows; r++) {
        ...
        for (const ex of exR) {
            ...
            if (exXs.length >= 2) {
                ...
                const exXsExp = exXs.map((x, k) => k % 2 === 0 ? x - EPS_EXCL : x + EPS_EXCL);
                segs = segs.flatMap(([lo, hi]) => subtractIntervals(lo, hi, exXsExp));
            }
        }
        ...
        for (const [lo, hi] of segs) {
            ...
            const c1 = Math.min(cols - 1, Math.floor((hi - xMin - 1e-9) / cellSize));
            for (let c = c0; c <= c1; c++) if (grid[r][c] === 0) grid[r][c] = 0.5;
        }
...
const N_KNN = 10; // k-nearest neighbours in Voronoi roadmap (empirically set, §4.4.3)
...
function delaunayTriangulation(points) {
    ...
    const pts = [...points, [mx - d, my - d * 2], [mx, my + d * 2], [mx + d, my - d * 2]];
    const mkTri = (i, j, k) => ({ v: [i, j, k], cc: circumcircle(pts[i], pts[j], pts[k]) });
        ...
        for (const t of badIdx) {
            ...
            for (const [e0, e1] of [[a, b], [b, c], [c, a]]) {
                ...
                for (const t2 of badIdx) {
                    if (t2 === t) continue;
                    const v2 = tris[t2].v;
                    if (v2.includes(e0) && v2.includes(e1)) { shared = true; break; }
                }
...
//  2. Voronoi vertices = circumcentres of Delaunay triangles (dual graph).
//  3. Filter to free-space vertices (inside perim, outside obstacles).
//  4. Connect each free vertex to N_KNN nearest free neighbours where the
//     edge is collision-free (local planner check).
// The roadmap is built once and reused for all inter-strip transitions.
function buildVoronoiRoadmap(perimR, exR, exRC, laneWidth, perimBBox, exBBoxes, exCBBoxes) {
    ...
    const getIdx = (cx, cy) => {
        ...
        if (!vertMap.has(k)) { vertMap.set(k, verts.length); verts.push([cx, cy]); }
    ...
    const isFree = verts.map(v =>
        pointInPoly(v, perimR) && !exRC.some(ex => pointInPoly(v, ex))
    );
    ...
    // Connect each free vertex to its N_KNN nearest collision-free free neighbours.
    ...
    for (const i of freeIdxs) {
        ...
        for (const j of freeIdxs) {
            if (j === i) continue;
            const d = Math.hypot(verts[j][0] - target[0], verts[j][1] - target[1]);
        ...
        for (const { j, d } of neighbors) {
            ...
            if (adj[i].some(e => e.v === j)) { connected++; continue; } // already linked
            if (segmentFree(verts[i], verts[j], perimR, exR, exRC, perimBBox, exBBoxes, exCBBoxes)) {
                adj[i].push({ v: j, w: d });
                adj[j].push({ v: i, w: d });
                connected++;
            }
...
function dijkstra(adj, src, dst, n) {
    ...
    for (let iter = 0; iter < n; iter++) {
        let u = -1;
        for (let k = 0; k < n; k++) if (!done[k] && (u === -1 || dist[k] < dist[u])) u = k;
...
function routeTransition(from, to, roadmap, perimR, perimR_tolerance, exR, exRC, perimBBox = null, perimBBox_tolerance = null, exBBoxes = [], exCBBoxes = []) {
    ...
    const n = pts.length;
    const adj = [[], []];
    for (let i = 0; i < ns; i++) adj.push(sa[i].map(({ v, w }) => ({ v: v + 2, w })));
        ...
        if (ex.type === 'circle') {
            ...
            if (dist < ex.r) {
                if (dist < 1e-9) {
                    ...
                    currentPt = [ex.cx + (dx / dist) * ex.r, ex.cy + (dy / dist) * ex.r];
                }
            }
        } else {
...
function removeSharpPeaks(pts) {
    ...
    while (changed && iterations < 1000) {
        ...
        for (let i = 1; i < N - 1; i++) {
            ...
            if (cosAngle < -0.5) { // angle > 120 degrees
                result.splice(i, 1);
                changed = true;
                break;
            }
...
function generateCoveragePath(perimCoords, exclusions, laneWidth, buffer, nPasses = 0, direction = 'CW', tolerance = 0, skipLanes = 0, sweepMode = 'auto', sweepAngle = 0) {
    ...
    if (perim.length > 1) {
        const [fx, fy] = perim[0], [lx, ly] = perim.at(-1);
    ...
    if (nPasses > 0) {
        perimBoustrophedonOrig = insetPoly(navigablePerimeter, (nPasses - 0.5) * laneWidth);
    }
    ...
    if (sweepMode === 'auto') {
        ...
        angle = isNaN(parsedAngle) ? 0 : parsedAngle * Math.PI / 180;
    }
    ...
    const exBBoxes  = exR.map(getBBox);
    const exCBBoxes = exRC.map(getBBox);

    // Associate rotated exclusions with their geometries in sweep space for direct point pushing
    const exData = exclusions.map((s, idx) => {
        const polyR = exR[idx];
        if (s.type === 'circle') {
            const [cx, cy] = toLocal(s.lat, s.lon, oLat, oLon);
    ...
    if (nPasses > 0) {
        let poly = [...navigablePerimeterRotated];
        ...
        const cx = perimOuterR.reduce((s, p) => s + p[0], 0) / perimOuterR.length;
        const cy = perimOuterR.reduce((s, p) => s + p[1], 0) / perimOuterR.length;
        ...
        const getExDataForDist = (dist) => {
            return exclusions.map((s, idx) => {
                ...
                const expanded = totalBuffer > 0 ? expandPoly(polyLocal, totalBuffer) : polyLocal;
                return {
                    type: 'polygon',
        ...
        for (let p = 0; p < nPasses; p++) {
            for (let i = 0; i < M; i++) {
                ...
                for (let k = 0; k < n; k++) {
                    ...
                    const sv = [sv1[0] + t * (sv2[0] - sv1[0]), sv1[1] + t * (sv2[1] - sv1[1])];
                    ...
                    const shiftLen = Math.hypot(offset[0] - pt[0], offset[1] - pt[1]);
                    const maxAllowed = Math.hypot(cx - pt[0], cy - pt[1]) * 0.9;
                    let currentPt = offset;
                    if (shiftLen > maxAllowed && shiftLen > 1e-9) {
                        const scale = maxAllowed / shiftLen;
                        currentPt = [pt[0] + (offset[0] - pt[0]) * scale, pt[1] + (offset[1] - pt[1]) * scale];
                    }

                    // Push outside exclusions and clamp to the corresponding yard inset
                    let finalPt = pushPointOutsideExclusions(currentPt, currentExData);
                    if (!pointInPoly(finalPt, currentYardInset)) {
                        finalPt = getClosestPointOnPoly(finalPt, currentYardInset);
                    }
            ...
            if (p === nPasses - 1) {
                ...
                let finalPt = pushPointOutsideExclusions(currentPt, exDataLaps[p]);
        ...
        perimeterPathRotated = condenseCollinearPoints(perimeterPathRotated, 0.01);
    }
    ...
    if (!strips.length) {
        if (nPasses > 0) {
            const path = safePerimeterPath.map(([x, y]) => fromLocal(x, y, oLat, oLon));
    ...
    if (nPasses > 0) {
        ...
        finalPathMetric = [...safePerimeterPath, ...transit.slice(1), ...boustrophedonPath.slice(1)];
    } else {
